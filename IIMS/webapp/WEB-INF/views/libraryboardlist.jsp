...
<%@page import="com.netcruz.iims.vo.LibraryBoardVo"%>
...
<body oncontextmenu="return false" ondragstart="return false" onselectstart="return false">
			...
			<div class="col-ms-3" >              
              Search <input ng-model="test" id="search" class="form-control" placeholder="Filter text">
				...
				<table class="table table-striped">
					...
					<tr ng-repeat="x in libraryboard | pagination: curPage * pageSize | limitTo: pageSize | filter:test "
					data-toggle="modal" ng-dblclick="do_some_action(x)" id="libraryboardtable">
						<td><div style="position:relative; width:100px; text-overflow:ellipsis; overflow:hidden; cursor:hand"><nobr>{{x.regDate}}</nobr></div></td>
						<td><div style="position:relative; width:150px; text-overflow:ellipsis; overflow:hidden; cursor:hand"><nobr>{{x.category}}</nobr></div></td>
						...
						<td><div style="position:relative; width:144px; text-overflow:ellipsis; overflow:hidden; cursor:hand"><nobr>{{x.userId}}</nobr></div></td>
						<td><div style="position:relative; width:200px; text-overflow:ellipsis; overflow:hidden; cursor:hand"><nobr><a href="fileDown.do?file={{x.file}}">{{x.file}}</a></nobr></div></td>
					</tr>
				</table>
				
				<center>
				<div class="pagination pagination-centered" ng-show="libraryboard.length">
					<ul class="pagination-controle pagination">
	 					...
	 					<li>
	 						<button type="button" class="btn btn-primary" ng-disabled="curPage >= libraryboard.length/pageSize - 1" ng-click="curPage = curPage+1">NEXT &gt;</button>
	 					</li>
					</ul>
				</div>
				</center>
			...
			<div class="modal" id="addWidgetModal3">
				<div class="modal-dialog modal-size">
					<div class="modal-content">
						...
						<form action="insertlibraryboard.do" method="post" id="eventForm" name="myForm" enctype="multipart/form-data">
							...
							<label>비고</label>
							<textarea name="note" class="form-control" >{{x.note}}</textarea><br>
							<label>첨부 파일</label> 
				...
				<div class="modal-dialog modal-size">
					<div class="modal-content">
						...
						<div class="modal-footer">
							...
							<a href="deletelibraryboard.do?id={{x.id}}"
				...
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body margin1">
							...
							<form action="updatelibraryboard.do" name="write" method="post" enctype="multipart/form-data">
								...
								<textarea  name="note" class="form-control" >{{x.note}}</textarea><br>
								<label>첨부 파일</label> 
								<input type="file" class="form-control" name="fileData"/><br><br><br><br><br><br><br><br>
						</div>
	...
	var myApp = angular.module('myApp',[]);
			...
			function UserCtrl($scope, $http) {
				...
				$http({
					method : 'GET',
					url : 'afterlibraryboardlist.do',
					headers : {
						'Content-type' : 'application/json'
					}
				}).success(function(data, status, headers, config) {
					$scope.libraryboard = data;
					$scope.curPage = 0;
					$scope.pageSize = 10;					
				    $scope.numberOfPages = function() {
					return Math.ceil($scope.libraryboard.length / $scope.pageSize);
				    };	
